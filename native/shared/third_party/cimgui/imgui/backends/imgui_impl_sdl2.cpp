...
bool ImGui_ImplSDL2_ProcessEvent(const SDL_Event* event)
{
    ...
    switch (event->type)
    {
        ...
        {
            ImVec2 mouse_pos(event->tfinger.x * io.DisplaySize.x, event->tfinger.y * io.DisplaySize.y);            
            if (io.ConfigFlags & ImGuiConfigFlags_ViewportsEnable)
            {
                int window_x, window_y;
                SDL_GetWindowPosition(SDL_GetWindowFromID(event->motion.windowID), &window_x, &window_y);
        ...
        case SDL_FINGERUP:
        {
            ...
            io.AddMouseButtonEvent(mouse_button, (event->type == SDL_FINGERDOWN));
            bd->MouseButtonsDown = (event->type == SDL_FINGERDOWN) ? (bd->MouseButtonsDown | (1 << mouse_button)) : (bd->MouseButtonsDown & ~(1 << mouse_button));
            return true;
        }
        case SDL_TEXTINPUT:
        {
